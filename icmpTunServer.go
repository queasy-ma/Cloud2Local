...
func (p *Server) Run() error {
	...
	//if err != nil {
	//	loggo.Error("Error creating socket: %v\n", err)
	//	return err
	//}
	//file := os.NewFile(uintptr(fd), "icmp_socket")
...
func (p *Server) processPacket(packet *Packet) {
	...
	//服务端send时仅为8时有用
	if packet.my.Rproto == -1 {
		loggo.Info("Ignoring packet with Rproto == -1: Rproto=%d", packet.my.Rproto)
		return
	}
